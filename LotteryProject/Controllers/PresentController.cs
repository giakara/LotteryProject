...
namespace LotteryProject.Server.Controllers
{
	...
	public class PresentController : ControllerBase
	{
		...
		{
			var allPresents = await _presentService.GetAllPresentsPaged(pagingParameters, cancellationToken);
			return Ok(allPresents);
		}
		[HttpGet]
		[Route("SearchPresent/{searchText}")]
		...
		{
			var searchPresents = await _presentService.SearchPresents(searchText, pagingParameters, cancellationToken);
			return Ok(searchPresents);
		}
	}
}
